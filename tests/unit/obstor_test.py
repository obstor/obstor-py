...
from obstor import __version__ as obstor_version
from obstor.helpers import _DEFAULT_USER_AGENT, BaseURL, check_bucket_name
...
class GetURLTests(TestCase):
    ...
    def test_obstor_requires_string(self):
        self.assertRaises(TypeError, Obstor, 10)

    def test_obstor_requires_hostname(self):
        with self.assertRaises(ValueError):
            Obstor(endpoint='http://')


class UserAgentTests(TestCase):
    def test_default_user_agent(self):
        client = Obstor(endpoint='localhost')
        self.assertEqual(client._user_agent, _DEFAULT_USER_AGENT)

    def test_set_app_info(self):
        client = Obstor(endpoint='localhost')
        expected_user_agent = _DEFAULT_USER_AGENT + ' hello/' + obstor_version
        client.set_app_info('hello', obstor_version)
    ...
    def test_set_app_info_requires_non_empty_name(self):
        ...
        self.assertRaises(ValueError, client.set_app_info, '', obstor_version)
