...
"""Obstor Admin Client to perform Obstor administration operations."""
...
from .error import ObstorAdminException
...
class ObstorAdmin:
    """Client to perform Obstor administration operations."""
    ...
    def _url_open(
        ...
        url = url_replace(url=self._url, path="/obstor/admin/v3/"+command.value)
    ...
    def top_locks(self) -> str:
        """Get a list of the 10 oldest locks on a Obstor cluster."""
        response = self._url_open(
            method="GET",
    ...
    def _attach_detach_policy(
        ...
        if (user is not None) ^ (group is not None):
            ...
            if (
                ...
                # Older Obstor servers do not return response.
                response.close()
                response.release_conn()
                return ""
